...
| `CLANGD_PATH` | `clangd` | Path to the clangd LSP binary (used by LSP tools for C/C++ analysis) |
...
| `clangd_completion` | `tools/tools_clangd.py` | List code completions at a cursor position via clangd; returns JSON completion list |
| `clangd_definition` | `tools/tools_clangd.py` | Jump to the definition of a symbol under the cursor; returns locations |
| `clangd_references` | `tools/tools_clangd.py` | Find all references to a symbol; returns locations |
| `clangd_document_symbols` | `tools/tools_clangd.py` | List symbols defined in a single file; returns JSON hierarchy |
| `clangd_workspace_symbols` | `tools/tools_clangd.py` | Search symbols across the workspace by query string |
| `clangd_rename` | `tools/tools_clangd.py` | Return a `WorkspaceEdit` JSON renaming a symbol |
| `clangd_type_hierarchy` | `tools/tools_clangd.py` | Explore supertypes / subtypes of a type; direction: `"subtypes"`, `"supertypes"`, or `"both"` |
| `clangd_call_hierarchy` | `tools/tools_clangd.py` | Explore incoming / outgoing calls; direction: `"incoming"`, `"outgoing"`, or `"both"` |

All filesystem and shell tools restrict access to paths inside the project working directory.

### LSP / C++ tools

The eight `clangd_*` tools speak the Language Server Protocol over stdio to a `clangd` subprocess. They are designed for C/C++ codebases and provide IDE-grade code intelligence (completions, navigation, refactoring, symbol search).

**Prerequisites:**

1. **clangd** must be installed and on `PATH` (or pointed to by the `CLANGD_PATH` env var). Install via your system package manager (e.g. `apt install clangd`, `brew install llvm`).
2. **`compile_commands.json`** in the project root — generated by CMake (`-DCMAKE_EXPORT_COMPILE_COMMANDS=ON`), Bear, or a build system that produces it. Without this file, clangd falls back to best-effort parsing and results may be incomplete or inaccurate.

**Behaviour:**

- The clangd subprocess is started lazily on the first tool call and shut down when the agent exits. The first call may take a few seconds (clangd index warm-up); subsequent calls are fast.
